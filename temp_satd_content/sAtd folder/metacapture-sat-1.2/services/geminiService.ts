...
export const analyzeContent = async (url: string, content: string, mode: CaptureMode): Promise<string | { v2Card: any; imageBase64: string }> => {
    ...
    if (mode === 'CharacterCard') {
        ...
        Based on this analysis and the user's wish: "${content}", create a detailed text description for a new character who would fit into that world. 
        The language of the character's descriptive fields must match the language of the user's wish.
        ...
        Also, if applicable, suggest a 'system_prompt' (compact rules/world constraints), a starting 'scenario', and 'creator_notes' (a short attribution).`;
        ...
        const characterSchema = {
            type: Type.OBJECT,
            properties: {
                name: { type: Type.STRING, description: "The character's name (max 40 chars)." },
                description: { type: Type.STRING, description: "A visually rich description of the character's appearance, clothing, and aura, suitable for generating an image (max 200 chars)." },
                personality: { type: Type.STRING, description: "A summary of the character's personality traits (max 220 chars)." },
                first_mes: { type: Type.STRING, description: "A characteristic first line of dialogue for the character (max 240 chars)." },
                system_prompt: { type: Type.STRING, description: "(Optional) Compact rules/world constraints for the character's behavior (max 280 chars)." },
                scenario: { type: Type.STRING, description: "(Optional) A starting situation or scenario for the character (max 220 chars)." },
                creator_notes: { type: Type.STRING, description: "(Optional) A short note from the creator or attribution (max 160 chars)." },
            },
        ...
        The language used in 'description', 'personality', and 'first_mes' should match the language of the original user wish: "${content}".
        Adhere strictly to the character limits for each field.
        The 'description' field must be visually rich and focus on appearance for image generation.
        'system_prompt', 'scenario', and 'creator_notes' are optional; only include them if relevant information can be extracted from the text.
